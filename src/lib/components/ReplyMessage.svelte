<script lang="ts">
	...
	let message = $derived(new CoState(Message, replyTo));
	let profile = $derived(
...
{#if message.current}
	<div class={cn('group relative flex w-full justify-start gap-3 bg-base-200/50 dark:bg-base-900/40 border border-base-300/50 dark:border-base-800/30 items-center text-xs rounded-2xl text-base-700 dark:text-base-300 px-3 py-0.5')}>
